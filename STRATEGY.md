...
**Public LB best**: `submissions/v110_park_p12_sh280.csv` → **3.00095 (rank 1)**, +0.00890 over rank 2.
...
The pipeline treats public and private rows identically — both halves flow through the same feature extraction, same model, same scaler. Verified post-facto: public vs private statistics are within 1% on both mean and std, so no public-only overfitting.

Private score will deviate from public only through sampling noise; there's no structural bias in our favor or against us.
