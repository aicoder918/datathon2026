"""Seed-5 ensemble CatBoost with hl_n_pos and hl_n_neg dropped.

Rationale: paired-diff -hl_counts Δ=+0.078 (t=+1.55), halves +0.082/+0.073 —
the most consistently positive ablation. These two columns are highly redundant
with hl_n and hl_mean_sent (they encode sign×count already captured by
net_sent).

Primary question: do feature REMOVALS transfer to LB, or do they fail the same
way additions have? One cheap test.
"""
...
N_CV_REPEATS = 5
N_SEEDS = 5
DROP_COLS = ["hl_n_pos", "hl_n_neg"]

X_full, y_full = load_train()
X_test = load_test()
X_full = X_full.drop(columns=DROP_COLS)
X_test = X_test.drop(columns=DROP_COLS)
print(f"Train: {X_full.shape}   Test: {X_test.shape}   Dropped: {DROP_COLS}")
...
sub_path = ROOT / "submissions" / "catboost_bars_seed5_slim.csv"
